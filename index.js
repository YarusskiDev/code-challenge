...
app.post('/pessoa/create',(req,res)=>{
    ...
    modelPessoa.findOne({where:{cpf:cpf}}).then((usuarioExistente=>{
        if(usuarioExistente == undefined){
            ...
            res.json({msgError:"Você já tem uma conta, caso não lembre sua senha, entre em contato com suporte"});
        }
    }));
...
app.put('/pessoa/transferir/:id',(req,res)=>{
    let id = parseInt(req.params.id);
            ...
            modelPessoa.findOne({
                ...
                if(pessoa != undefined){
                    ...
                    if(pessoa.saldo >= 0){
                       ...
                       return res.json({msgError:"Você não tem saldo suficiente para realizar essa transação"});
                    }
                }
                modelPessoa.findOne({where:{id:idDestinatario}})
                .then((destinatario)=>{
...
app.put('/pessoa/deposito/:id',(req,res)=>{
    let id = parseInt(req.params.id);
